...
- **Interactive mapping:** Folium and Streamlit render campus maps and route results with street, Esri satellite, and optional Planet imagery layers.
...
| `GEMINI_MODEL` | Optional | Gemini model identifier; defaults to `gemini-3.5-flash-lite`. |
| `PLANET_API_KEY` | Optional | Authenticates Planet tile requests when a mosaic ID is configured. |
| `PLANET_MOSAIC_ID` | Optional | Planet Basemaps mosaic ID for the optional Planet imagery layer. |
